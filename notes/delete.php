...
if (!$note = note_load($noteid)) {
    throw new \powereduc_exception('invalidid');
}
...
if (!has_capability('powereduc/notes:manage', $context)) {
    throw new \powereduc_exception('nopermissiontodelete', 'notes');
}
...
if (data_submitted() && confirm_sesskey()) {
    ...
    $PAGE->navbar->add(fullname($user), new powereduc_url('/user/view.php', array('id' => $user->id, 'course' => $course->id)));
    $PAGE->navbar->add(get_string('notes', 'notes'),
                       new powereduc_url('/notes/index.php', array('user' => $user->id, 'course' => $course->id)));
    $PAGE->navbar->add(get_string('delete'));
    $PAGE->set_title($course->shortname . ': ' . $strnotes);
    $PAGE->set_heading($course->fullname);
    echo $OUTPUT->header();
    echo $OUTPUT->confirm(get_string('deleteconfirm', 'notes'),
                          new powereduc_url('delete.php', $optionsyes),
