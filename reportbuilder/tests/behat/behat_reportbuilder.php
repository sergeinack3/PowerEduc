...
class behat_reportbuilder extends behat_base {
    ...
    protected function resolve_page_instance_url(string $type, string $identifier): powereduc_url {
        ...
        switch ($type) {
            case 'Editor':
                return new powereduc_url('/reportbuilder/edit.php', ['id' => $report->get('id')]);

            case 'View':
                return new powereduc_url('/reportbuilder/view.php', ['id' => $report->get('id')]);
    ...
    public function i_set_the_column_aggregation_to(string $column, string $aggregation): void {
        ...
        if ($aggregation === $aggregationgroupconcatdistinct && !groupconcatdistinct::compatible(column::TYPE_TEXT)) {
            throw new \PowerEduc\BehatExtension\Exception\SkippedException("{$aggregationgroupconcatdistinct} not available");
        }
