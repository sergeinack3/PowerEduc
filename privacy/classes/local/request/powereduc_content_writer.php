...
/**
 * The powereduc_content_writer is the default PowerEduc implementation of a content writer.
 *
 * It exports data to a rich tree structure using PowerEduc's context system,
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class powereduc_content_writer implements content_writer {
    /**
     ...
     * @throws  \powereduc_exception           If the file cannot be written for some reason.
     */
    protected function write_data(string $path, string $data) {
        ...
        if (file_put_contents($targetpath, $data) === false) {
            throw new \powereduc_exception('cannotsavefile', 'error', '', $targetpath);
        }
    ...
    protected function make_tree_object(array $tree, array $treekey, array $currentkey = []) : Array {
        ...
        $addfinalfile = function($directory, $treeleaf, $file) use ($treekey) {
            ...
            if (isset($treekey[$gokey]) && $treekey[$gokey] !== 'No var') {
                ...
                $treeleaf->url = new \powereduc_url($url . '/' . $file);
            }
        };
    ...
    protected function write_html_data() {
        ...
        $powereducimgpath = ['pix', 'powereduclogo.svg'];
        $this->copy_data($powereducimgpath, ['pix', 'powereduclogo.svg']);

        // Additional required css.
        $csspath = ['theme', 'boost', 'style', 'powereduc.css'];
        $destination = ['powereduc.css'];
     ...
     * @throws \powereduc_exception If the file cannot be opened.
     */
    protected function get_file_content(string $filepath) : String {
        $content = file_get_contents($filepath);
        if ($content === false) {
            throw new \powereduc_exception('cannotopenfile', 'error', '', $filepath);
        }
        return $content;
    }
}
