...
use \core_privacy\local\request\powereduc_content_writer;
 ...
 * @copyright   2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @coversDefaultClass \core_privacy\local\request\powereduc_content_writer
 */
class powereduc_content_writer_test extends advanced_testcase {
     ...
     *
     * @return  powereduc_content_writer
     */
    public function get_writer_instance() {
        $factory = $this->createMock(writer::class);
        return new powereduc_content_writer($factory);
    }

    /**
     * Fetch the exported content for inspection.
     *
     * @param   powereduc_content_writer   $writer
     * @return  \org\bovigo\vfs\vfsStreamDirectory
     */
    protected function fetch_exported_content(powereduc_content_writer $writer) {
        $export = $writer
    ...
    protected function get_context_path($context, $subcontext = null, $name = '') {
        $rc = new ReflectionClass(powereduc_content_writer::class);
        $writer = $this->get_writer_instance();
    ...
    public function test_export_html_functions() {
        ...
        list($tree, $treelist, $indexdata) = phpunit_util::call_internal_method($writer, 'prepare_for_export', [],
                '\core_privacy\local\request\powereduc_content_writer');
                            ...
                            'tests' => (object) [
                                ...
                                'children' => [
                                    'a.txt' => (object) [
                                        'name' => 'a.txt',
                                        'itemtype' => 'item',
                                        'url' => new \powereduc_url('System _.1/_files/tests/a.txt')
                                    ]
                                ]
                            ]
                        ]
