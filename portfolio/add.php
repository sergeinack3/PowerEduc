...
if (!empty($dataid)) {
    ...
    if ($cancel) {
        if ($cancelsure) {
            ...
            $yesbutton = new single_button(new powereduc_url('/portfolio/add.php', array('id' => $dataid, 'cancel' => 1, 'cancelsure' => 1, 'logreturn' => $logreturn)), get_string('yes'));
            if ($logreturn) {
                $nobutton  = new single_button(new powereduc_url('/user/portfoliologs.php'), get_string('no'));
            } else {
                $nobutton  = new single_button(new powereduc_url('/portfolio/add.php', array('id' => $dataid)), get_string('no'));
            }
...
// completely new request, look to see what information we've been passed and set up the exporter object.
    ...
    // without knowing information about what part of powereduc we come from.
    if (empty($callbackcomponent) || empty($callbackclass)) {
        debugging('no callback file or class');
        portfolio_exporter::print_expired_export();
    }

    // so each place in powereduc can pass callback args here
