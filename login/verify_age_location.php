...
if (!$authplugin || !\core_auth\digital_consent::is_age_digital_consent_verification_enabled()) {
    ...
    redirect(new powereduc_url('/'), get_string('verifyagedigitalconsentnotpossible', 'error'));
}

$PAGE->set_context(context_system::instance());
$PAGE->set_url(new powereduc_url('/login/verify_age_location.php'));

if (isloggedin() and !isguestuser()) {
    // Prevent signing up when already logged in.
    redirect(new powereduc_url('/'), get_string('cannotsignup', 'error', fullname($USER)));
}
...
if ($isminor === 'yes') {
    ...
    redirect(new powereduc_url('/login/digital_minor.php'));
} else if ($isminor === 'no') {
    // The user that attempts to sign up has already verified that they are not a digital minor.
    redirect(new powereduc_url('/login/signup.php'));
}
