...
class action_bar implements templatable, renderable {
     ...
     */
    public function __construct(int $pageid, powereduc_url $pageurl, bool $displayprint = false) {
        $this->pageid = $pageid;
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($this->displayprint) {
            $printlink = new powereduc_url('/mod/wiki/prettyview.php', ['pageid' => $this->pageid]);
    ...
    private function get_action_selector(): \url_select {
        ...
        if (has_capability('mod/wiki:viewpage', $PAGE->context)) {
            $viewlink = new powereduc_url('/mod/wiki/view.php', ['pageid' => $this->pageid]);
            $menu[$viewlink->out(false)] = get_string('view', 'mod_wiki');
        }

        if (has_capability('mod/wiki:editpage', $PAGE->context)) {
            $editlink = new powereduc_url('/mod/wiki/edit.php', ['pageid' => $this->pageid]);
            $menu[$editlink->out(false)] = get_string('edit', 'mod_wiki');
        }

        if (has_capability('mod/wiki:viewcomment', $PAGE->context)) {
            $commentslink = new powereduc_url('/mod/wiki/comments.php', ['pageid' => $this->pageid]);
            $menu[$commentslink->out(false)] = get_string('comments', 'mod_wiki');
        }

        if (has_capability('mod/wiki:viewpage', $PAGE->context)) {
            $historylink = new powereduc_url('/mod/wiki/history.php', ['pageid' => $this->pageid]);
            $menu[$historylink->out(false)] = get_string('history', 'mod_wiki');
        }

        if (has_capability('mod/wiki:viewpage', $PAGE->context)) {
            $maplink = new powereduc_url('/mod/wiki/map.php', ['pageid' => $this->pageid]);
            $menu[$maplink->out(false)] = get_string('map', 'mod_wiki');
        }

        if (has_capability('mod/wiki:viewpage', $PAGE->context)) {
            $fileslink = new powereduc_url('/mod/wiki/files.php', ['pageid' => $this->pageid]);
            $menu[$fileslink->out(false)] = get_string('files', 'mod_wiki');
        }

        if (has_capability('mod/wiki:managewiki', $PAGE->context)) {
            $adminlink = new powereduc_url('/mod/wiki/admin.php', ['pageid' => $this->pageid]);
