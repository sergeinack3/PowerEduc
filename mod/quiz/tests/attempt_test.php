...
class attempt_test extends \advanced_testcase {
    ...
    public function test_attempt_url() {
        ...
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->attempt_url(null, 2));

        $params['page'] = 1;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->attempt_url(3));

        $questionattempt = $attempt->get_question_attempt(4);
        $expecteanchor = $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url($url, $params, $expecteanchor), $attempt->attempt_url(4));

        $questionattempt = $attempt->get_question_attempt(3);
        $expecteanchor = '#' . $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url('#'), $attempt->attempt_url(null, 2, 2));
        $this->assertEquals(new \powereduc_url($expecteanchor), $attempt->attempt_url(3, -1, 1));

        $questionattempt = $attempt->get_question_attempt(4);
        $expecteanchor = $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url(null, null, $expecteanchor, null), $attempt->attempt_url(4, -1, 1));

        // Summary page.
        $url = '/mod/quiz/summary.php';
        unset($params['page']);
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->summary_url());

        // Review page.
        $url = '/mod/quiz/review.php';
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url());

        $params['page'] = 1;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(3, -1, false));
        $this->assertEquals(new \powereduc_url($url, $params, $expecteanchor), $attempt->review_url(4, -1, false));

        unset($params['page']);
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(null, 2, true));
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(1, -1, true));

        $params['page'] = 2;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(null, 2, false));
        unset($params['page']);

        $params['showall'] = 0;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(null, 0, false));
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(1, -1, false));
        ...
        $params['page'] = 2;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(null, 2));
        $this->assertEquals(new \powereduc_url('#'), $attempt->review_url(null, -1, null, 0));

        $questionattempt = $attempt->get_question_attempt(3);
        $expecteanchor = '#' . $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url($expecteanchor), $attempt->review_url(3, -1, null, 0));

        $questionattempt = $attempt->get_question_attempt(4);
        $expecteanchor = '#' . $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url($expecteanchor), $attempt->review_url(4, -1, null, 0));
        $this->assertEquals(new \powereduc_url('#'), $attempt->review_url(null, 2, true, 0));

        $questionattempt = $attempt->get_question_attempt(1);
        $expecteanchor = '#' . $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url($expecteanchor), $attempt->review_url(1, -1, true, 0));
        $this->assertEquals(new \powereduc_url($expecteanchor), $attempt->review_url(1, -1, false, 0));
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(null, 2, false, 0));
        $this->assertEquals(new \powereduc_url('#'), $attempt->review_url(null, 0, false, 0));

        $params['page'] = 1;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(3, -1, false, 0));
        ...
        $params['page'] = 1;
        unset($params['showall']);
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(11, -1, false));

        $questionattempt = $attempt->get_question_attempt(12);
        $expecteanchor = $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url($url, $params, $expecteanchor), $attempt->review_url(12, -1, false));
        ...
        $questionattempt = $attempt->get_question_attempt(3);
        $expecteanchor = $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url(null, null, $expecteanchor), $attempt->review_url(3, -1, null, 0));

        $questionattempt = $attempt->get_question_attempt(4);
        $expecteanchor = $questionattempt->get_outer_question_div_unique_id();
        $this->assertEquals(new \powereduc_url(null, null, $expecteanchor), $attempt->review_url(4, -1, null, 0));
        ...
        $params['page'] = 1;
        $this->assertEquals(new \powereduc_url($url, $params), $attempt->review_url(11, -1, false, 0));
    }
