...
class restore_quiz_activity_structure_step extends restore_questions_activity_structure_step {
     ...
     * PowerEduc 2.9 quiz sections did not exist, so if the file being restored
    ...
    protected function process_quiz($data) {
        ...
        // Process Safe Exam Browser settings for backups taken in PowerEduc < 3.9.
        if (!empty($addsebrule)) {
    ...
    protected function process_quiz_attempt($data) {
        ...
        if (!isset($data->gradednotificationsenttime)) {
            // For attempts restored from old PowerEduc sites before this field
