...
class restore_lesson_activity_structure_step extends restore_activity_structure_step {
    ...
    protected function process_lesson_highscore($data) {
        ...
        // high scores were removed in PowerEduc 3.0 See MDL-49581.
    }
    ...
    protected function after_execute() {
        ...
        $badanswers = $DB->get_recordset_sql($sql, array('lessonid' => $this->task->get_activityid(), 'format' => FORMAT_POWEREDUC));

        foreach ($badanswers as $badanswer) {
            // Strip tags from answer text and convert back the format to FORMAT_POWEREDUC.
            $badanswer->answer = strip_tags($badanswer->answer);
            $badanswer->answerformat = FORMAT_POWEREDUC;
