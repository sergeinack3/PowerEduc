...
class lesson_page_type_endofbranch extends lesson_page {
    ...
    public function redirect_to_first_answer($canmanage, $redirect) {
        ...
        if ($redirect) {
            redirect(new powereduc_url('/mod/lesson/view.php', array('id' => $PAGE->cm->id, 'pageid' => $jumpto)));
            die;
        }
        return $jumpto;
    }
    ...
    public function add_page_link($previd) {
        global $PAGE, $CFG;
        if ($previd != 0) {
            $addurl = new powereduc_url('/mod/lesson/editpage.php', array('id'=>$PAGE->cm->id, 'pageid'=>$previd, 'sesskey'=>sesskey(), 'qtype'=>LESSON_PAGE_ENDOFBRANCH));
...
class lesson_add_page_form_endofbranch extends lesson_add_page_form_base {
    ...
    public function construction_override($pageid, lesson $lesson) {
        ...
        if (!$page = $DB->get_record("lesson_pages", array("id" => $pageid))) {
            throw new \powereduc_exception('cannotfindpagerecord', 'lesson');
        }
