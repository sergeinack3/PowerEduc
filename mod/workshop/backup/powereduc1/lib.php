 ...
 */
class powereduc1_mod_workshop_handler extends powereduc1_mod_handler {
    ...
    /** @var powereduc1_file_manager for the current workshop */
    protected $fileman = null;

    /** @var powereduc1_inforef_manager */
    protected $inforefman = null;

    /** @var array list of powereduc1_workshopform_handler instances */
     ...
     *
     * Note that the path /POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP does not
    ...
    public function get_paths() {
        return array(
            new convert_path('workshop', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP'),
            new convert_path('workshop_elements', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS'),
            new convert_path(
                'workshop_element', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT',
                array(
                    'dropfields' => array(
                        'stddev',
                        'totalassessments',
            ...
            new convert_path('workshop_element_rubric', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP/ELEMENTS/ELEMENT/RUBRICS/RUBRIC'),
        );
    }

    /**
     * This is executed every time we have one /POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP
     * data available
     */
    public function process_workshop($data, $raw) {
        ...
        $this->currentworkshop['intro'] = powereduc1_converter::migrate_referenced_files($this->currentworkshop['intro'], $this->fileman);
    ...
    /**
     * Processes one <ELEMENT> tag from powereduc.xml
     */
    ...
    /**
     * Processes one <RUBRIC> tag from powereduc.xml
     */
     ...
     * @return powereduc1_workshopform_handler the instance of the handler
     */
    protected function get_strategy_handler($strategy) {
        ...
        if (is_null($this->strategyhandlers)) {
            ...
            foreach ($subplugins as $name => $dir) {
                ...
                $handlerclass = "powereduc1_workshopform_{$name}_handler";
                ...
                if (!$this->strategyhandlers[$name] instanceof powereduc1_workshopform_handler) {
                    throw new powereduc1_convert_exception('wrong_handler_class', get_class($this->strategyhandlers[$name]));
                }
            }
        }

        if (!isset($this->strategyhandlers[$strategy])) {
            throw new powereduc1_convert_exception('usupported_subplugin', 'workshopform_'.$strategy);
        }
 ...
 */
abstract class powereduc1_workshopform_handler extends powereduc1_submod_handler {

    /**
     * @param powereduc1_mod_handler $workshophandler the handler of a module we are subplugin of
     * @param string $subpluginname the name of the subplugin
     */
    public function __construct(powereduc1_mod_handler $workshophandler, $subpluginname) {
