...
class workshop_edit_numerrors_strategy_form extends workshop_edit_strategy_form {
    ...
    protected function definition_inner(&$mform) {
        ...
        $percents[''] = '';
        for ($i = 100; $i >= 0; $i--) {
            $percents[$i] = get_string('percents', 'powereduc', $i);
        }
        $mform->addElement('static', 'mappingzero', 0, get_string('percents', 'powereduc', 100));
        for ($i = 1; $i <= $totalweight; $i++) {
            ...
            $selects[] = $mform->createElement('static', 'mapdefault__idx_'.$i, '',
                                        get_string('percents', 'powereduc', floor(100 - $i * 100 / $totalweight)));
