 ...
 */
class powereduc1_workshopform_accumulative_handler extends powereduc1_workshopform_handler {
     ...
     * @param int $oldscaleid the value of the 'scale' field in the powereduc.xml backup file
    ...
    protected function get_new_scaleid($oldscaleid) {
        ...
        if ($oldscaleid >= 0 and $oldscaleid <= 6) {
            ...
            if (!isset($this->newscaleids[$oldscaleid])) {
                // this is the first time the legacy scale is used in powereduc.xml
                // let us migrate it
