...
if ($canmanage) {
    ...
    throw new \powereduc_exception('nopermissions', 'error', $workshop->view_url(), 'compare example assessment');
}
...
if ($isreviewer) {
    ...
    if ($workshop->assessing_examples_allowed()) {
        $assessment->add_action(
            new powereduc_url($workshop->exsubmission_url($example->id), array('assess' => 'on', 'sesskey' => sesskey())),
