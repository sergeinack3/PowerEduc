...
/**
 * Handles the conversion of /POWEREDUC_BACKUP/INFO paths
 *
...
class powereduc1_info_handler extends powereduc1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('info', '/POWEREDUC_BACKUP/INFO'),
            new convert_path('info_details', '/POWEREDUC_BACKUP/INFO/DETAILS'),
            new convert_path('info_details_mod', '/POWEREDUC_BACKUP/INFO/DETAILS/MOD'),
            new convert_path('info_details_mod_instance', '/POWEREDUC_BACKUP/INFO/DETAILS/MOD/INSTANCES/INSTANCE'),
        );
    }
...
class powereduc1_course_header_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'course_header', '/POWEREDUC_BACKUP/COURSE/HEADER',
                array(
                    'newfields' => array(
                        'summaryformat'          => 1,
                        'legacyfiles'            => 2,
            ...
            new convert_path(
                'course_header_category', '/POWEREDUC_BACKUP/COURSE/HEADER/CATEGORY',
                array(
                    'newfields' => array(
                        'description' => null,
                    )
...
class powereduc1_course_outline_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('course_sections', '/POWEREDUC_BACKUP/COURSE/SECTIONS'),
            new convert_path(
                'course_section', '/POWEREDUC_BACKUP/COURSE/SECTIONS/SECTION',
                array(
                    'newfields' => array(
                        'name'          => null,
                        'summaryformat' => 1,
            ...
            new convert_path(
                'course_module', '/POWEREDUC_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD',
                array(
                    'newfields' => array(
                        'completion'                => 0,
            ...
            new convert_path('course_modules', '/POWEREDUC_BACKUP/COURSE/MODULES'),
            // todo new convert_path('course_module_roles_overrides', '/POWEREDUC_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_OVERRIDES'),
            // todo new convert_path('course_module_roles_assignments', '/POWEREDUC_BACKUP/COURSE/SECTIONS/SECTION/MODS/MOD/ROLES_ASSIGNMENTS'),
        );
    }
...
class powereduc1_roles_definition_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('roles', '/POWEREDUC_BACKUP/ROLES'),
            new convert_path(
                'roles_role', '/POWEREDUC_BACKUP/ROLES/ROLE',
                array(
                    'newfields' => array(
                        'description'   => '',
                        'sortorder'     => 0,
...
class powereduc1_question_bank_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        ...
        $paths = array(
            new convert_path('question_categories', '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES'),
            new convert_path(
                'question_category', '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY',
                array(
                    'newfields' => array(
                        'infoformat' => 0
                    )
                )),
            new convert_path('question_category_context', '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/CONTEXT'),
            new convert_path('questions', '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS'),
            // the question element must be grouped so we can re-dispatch it to the qtype handler as a whole
            new convert_path('question', '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION', array(), true),
        );
        ...
        foreach (array_keys($subpaths) as $subpath) {
            ...
            $path = '/POWEREDUC_BACKUP/COURSE/QUESTION_CATEGORIES/QUESTION_CATEGORY/QUESTIONS/QUESTION/'.$subpath;
...
class powereduc1_scales_handler extends powereduc1_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('scales', '/POWEREDUC_BACKUP/COURSE/SCALES'),
            new convert_path(
                'scale', '/POWEREDUC_BACKUP/COURSE/SCALES/SCALE',
                array(
                    'renamefields' => array(
                        'scaletext' => 'scale',
                    ),
...
class powereduc1_outcomes_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook_grade_outcomes', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES'),
            new convert_path(
                'gradebook_grade_outcome', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_OUTCOMES/GRADE_OUTCOME',
                array(
                    'addfields' => array(
...
class powereduc1_gradebook_handler extends powereduc1_xml_handler {
    ...
    public function get_paths() {
        return array(
            new convert_path('gradebook', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK'),
            new convert_path('gradebook_grade_letter', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_LETTERS/GRADE_LETTER'),
            new convert_path(
                'gradebook_grade_category', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_CATEGORIES/GRADE_CATEGORY',
                array(
                    'addfields' => array(
            ...
            new convert_path('gradebook_grade_item', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM'),
            new convert_path('gradebook_grade_item_grades', '/POWEREDUC_BACKUP/COURSE/GRADEBOOK/GRADE_ITEMS/GRADE_ITEM/GRADE_GRADES'),
        );
    }
...
abstract class powereduc1_qtype_handler extends powereduc1_plugin_handler {
    ...
    protected function get_default_numerical_options($oldquestiontextformat, $units) {
        ...
        if ($CFG->texteditors !== 'textarea' and $oldquestiontextformat == FORMAT_POWEREDUC) {
    ...
    private function convert_answer(array $old, $qtype) {
        ...
        if ($CFG->texteditors !== 'textarea') {
            ...
            $new['feedbackformat'] = FORMAT_POWEREDUC;
        }
...
abstract class powereduc1_block_handler extends powereduc1_plugin_handler {
    ...
    public function get_paths() {
        ...
        return array(
            new convert_path('block', "/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK/{$blockname}"),
        );
    }
