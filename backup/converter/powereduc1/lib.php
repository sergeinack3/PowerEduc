...
class powereduc1_converter extends base_converter {
    ...
    public static function detect_format($tempdir) {
        ...
        if (file_exists($filepath)) {
            ...
            if (strpos($first_chars,'<?xml version="1.0" encoding="UTF-8"?>') !== false and
                strpos($first_chars,'<POWEREDUC_BACKUP>') !== false and
                strpos($first_chars,'<INFO>') !== false) {

                return backup::FORMAT_POWEREDUC1;
            }
        }
    ...
    protected function init() {
        ...
        $this->xmlprocessor->add_path('/POWEREDUC_BACKUP/COURSE/MODULES/MOD');
    ...
    protected function register_handler(powereduc1_handler $handler, array $elements) {
        ...
        foreach ($elements as $element) {
            ...
            $path = preg_replace('/^\/POWEREDUC_BACKUP\/COURSE\/MODULES\/MOD\/(\w+)\//', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/', $path);
            $path = preg_replace('/^\/POWEREDUC_BACKUP\/COURSE\/BLOCKS\/BLOCK\/(\w+)\//', '/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK/', $path);
    ...
    public function process_chunk($data) {
        ...
        if ($path === '/POWEREDUC_BACKUP/COURSE/MODULES/MOD') {
            $this->currentmod = strtoupper($data['tags']['MODTYPE']);
            $path = '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod;

        } else if (strpos($path, '/POWEREDUC_BACKUP/COURSE/MODULES/MOD') === 0) {
            $path = str_replace('/POWEREDUC_BACKUP/COURSE/MODULES/MOD', '/POWEREDUC_BACKUP/COURSE/MODULES/MOD/' . $this->currentmod, $path);
        }

        // expand the BLOCK paths so that they contain the module name
        if ($path === '/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK') {
            $this->currentblock = strtoupper($data['tags']['NAME']);
            $path = '/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock;

        } else if (strpos($path, '/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK') === 0) {
            $path = str_replace('/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK', '/POWEREDUC_BACKUP/COURSE/BLOCKS/BLOCK/' . $this->currentblock, $path);
        }
     ...
     * /POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP because the <MOD> must
     ...
     * /POWEREDUC_BACKUP/COURSE/MODULES/MOD/WORKSHOP/SUBMISSIONS because it is
