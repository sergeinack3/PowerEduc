...
/**
 * Tests for powereduc_url.
 *
 * @package     core
 * @copyright   2018 Andrew Nicols <andrew@nicols.co.uk>
 * @license     http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class powereduc_url_test extends \advanced_testcase {
    /**
     * Test basic powereduc_url construction.
     */
    public function test_powereduc_url_constructor() {
        global $CFG;

        $url = new \powereduc_url('/index.php');
        $this->assertSame($CFG->wwwroot.'/index.php', $url->out());

        $url = new \powereduc_url('/index.php', array());
        $this->assertSame($CFG->wwwroot.'/index.php', $url->out());

        $url = new \powereduc_url('/index.php', array('id' => 2));
        $this->assertSame($CFG->wwwroot.'/index.php?id=2', $url->out());

        $url = new \powereduc_url('/index.php', array('id' => 'two'));
        $this->assertSame($CFG->wwwroot.'/index.php?id=two', $url->out());

        $url = new \powereduc_url('/index.php', array('id' => 1, 'cid' => '2'));
        $this->assertSame($CFG->wwwroot.'/index.php?id=1&amp;cid=2', $url->out());
        $this->assertSame($CFG->wwwroot.'/index.php?id=1&cid=2', $url->out(false));

        $url = new \powereduc_url('/index.php', null, 'test');
        $this->assertSame($CFG->wwwroot.'/index.php#test', $url->out());

        $url = new \powereduc_url('/index.php', array('id' => 2), 'test');
        $this->assertSame($CFG->wwwroot.'/index.php?id=2#test', $url->out());
    }

    /**
     * Tests \powereduc_url::get_path().
     */
    public function test_powereduc_url_get_path() {
        $url = new \powereduc_url('http://www.example.org:447/my/file/is/here.txt?really=1');
        $this->assertSame('/my/file/is/here.txt', $url->get_path());

        $url = new \powereduc_url('http://www.example.org/');
        $this->assertSame('/', $url->get_path());

        $url = new \powereduc_url('http://www.example.org/pluginfile.php/slash/arguments');
        $this->assertSame('/pluginfile.php/slash/arguments', $url->get_path());
        $this->assertSame('/pluginfile.php', $url->get_path(false));
    }

    public function test_powereduc_url_round_trip() {
        $strurl = 'http://powereduc.org/course/view.php?id=5';
        $url = new \powereduc_url($strurl);
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://powereduc.org/user/index.php?contextid=53&sifirst=M&silast=D';
        $url = new \powereduc_url($strurl);
        $this->assertSame($strurl, $url->out(false));
    }

    /**
     * Test PowerEduc URL objects created with a param with empty value.
     */
    public function test_powereduc_url_empty_param_values() {
        $strurl = 'http://powereduc.org/course/view.php?id=0';
        $url = new \powereduc_url($strurl, array('id' => 0));
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://powereduc.org/course/view.php?id';
        $url = new \powereduc_url($strurl, array('id' => false));
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://powereduc.org/course/view.php?id';
        $url = new \powereduc_url($strurl, array('id' => null));
        $this->assertSame($strurl, $url->out(false));

        $strurl = 'http://powereduc.org/course/view.php?id';
        $url = new \powereduc_url($strurl, array('id' => ''));
        $this->assertSame($strurl, $url->out(false));
    ...
    /**
     * Test set good scheme on PowerEduc URL objects.
     */
    public function test_powereduc_url_set_good_scheme() {
        $url = new \powereduc_url('http://powereduc.org/foo/bar');
        $url->set_scheme('myscheme');
        $this->assertSame('myscheme://powereduc.org/foo/bar', $url->out());
    }

    /**
     * Test set bad scheme on PowerEduc URL objects.
     */
    public function test_powereduc_url_set_bad_scheme() {
        $url = new \powereduc_url('http://powereduc.org/foo/bar');
        $this->expectException(\coding_exception::class);
        $url->set_scheme('not a valid $ scheme');
    }

    public function test_powereduc_url_round_trip_array_params() {
        $strurl = 'http://example.com/?a%5B1%5D=1&a%5B2%5D=2';
        $url = new \powereduc_url($strurl);
        $this->assertSame($strurl, $url->out(false));

        $url = new \powereduc_url('http://example.com/?a[1]=1&a[2]=2');
        $this->assertSame($strurl, $url->out(false));

        // For un-keyed array params, we expect 0..n keys to be returned.
        $strurl = 'http://example.com/?a%5B0%5D=0&a%5B1%5D=1';
        $url = new \powereduc_url('http://example.com/?a[]=0&a[]=1');
        $this->assertSame($strurl, $url->out(false));
    }

    public function test_compare_url() {
        $url1 = new \powereduc_url('index.php', array('var1' => 1, 'var2' => 2));
        $url2 = new \powereduc_url('index2.php', array('var1' => 1, 'var2' => 2, 'var3' => 3));
        ...
        $url2 = new \powereduc_url('index.php', array('var1' => 1, 'var3' => 3));

        $this->assertTrue($url1->compare($url2, URL_MATCH_BASE));
        $this->assertFalse($url1->compare($url2, URL_MATCH_PARAMS));
        $this->assertFalse($url1->compare($url2, URL_MATCH_EXACT));

        $url2 = new \powereduc_url('index.php', array('var1' => 1, 'var2' => 2, 'var3' => 3));

        $this->assertTrue($url1->compare($url2, URL_MATCH_BASE));
        $this->assertTrue($url1->compare($url2, URL_MATCH_PARAMS));
        $this->assertFalse($url1->compare($url2, URL_MATCH_EXACT));

        $url2 = new \powereduc_url('index.php', array('var2' => 2, 'var1' => 1));
    ...
    public function test_out_as_local_url_error() {
        $url2 = new \powereduc_url('http://www.google.com/lib/tests/weblib_test.php');
        $this->expectException(\coding_exception::class);
    ...
    public function test_modified_url_out_as_local_url_error() {
        ...
        $url3 = new \powereduc_url($modifiedurl.'/login/profile.php');
        $this->expectException(\coding_exception::class);
    ...
    public function test_https_out_as_local_url_error() {
        $url4 = new \powereduc_url('https://www.google.com/lib/tests/weblib_test.php');
        $this->expectException(\coding_exception::class);
        $url4->out_as_local_url();
    }

    public function test_powereduc_url_get_scheme() {
        // Should return the scheme only.
        $url = new \powereduc_url('http://www.example.org:447/my/file/is/here.txt?really=1');
        $this->assertSame('http', $url->get_scheme());

        // Should work for secure URLs.
        $url = new \powereduc_url('https://www.example.org:447/my/file/is/here.txt?really=1');
        $this->assertSame('https', $url->get_scheme());

        // Should return an empty string if no scheme is specified.
        $url = new \powereduc_url('www.example.org:447/my/file/is/here.txt?really=1');
        $this->assertSame('', $url->get_scheme());
    }

    public function test_powereduc_url_get_host() {
        // Should return the host part only.
        $url = new \powereduc_url('http://www.example.org:447/my/file/is/here.txt?really=1');
        $this->assertSame('www.example.org', $url->get_host());
    }

    public function test_powereduc_url_get_port() {
        ...
        $url = new \powereduc_url('http://www.example.org/some/path/here.php');
    ...
    public function test_make_pluginfile_url($slashargs, $args, $expected) {
        ...
        $url = call_user_func_array('\powereduc_url::make_pluginfile_url', $args);
    ...
    public function make_pluginfile_url_provider() {
        $baseurl = "https://www.example.com/powereduc/pluginfile.php";
        $tokenbaseurl = "https://www.example.com/powereduc/tokenpluginfile.php";
        return [
