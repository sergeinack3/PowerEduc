YUI.add('powereduc-editor_atto-plugin', function (Y, NAME) {
 ...
 * Atto editor plugin.
 *
 * @module powereduc-editor_atto-plugin
...
/**
 * A Plugin for the Atto Editor used in PowerEduc.
 *
...
var DISABLED = 'disabled',
    HIGHLIGHT = 'highlight',
    LOGNAME = 'powereduc-editor_atto-editor-plugin',
    CSS = {
...
EditorPluginButtons.prototype = {
    ...
    addBasicButton: function(config) {
        if (!config.exec) {
            Y.log('No exec command specified. Cannot proceed.',
                    'warn', 'powereduc-editor_atto-plugin');
            return null;
        }
    ...
    _normalizeIcon: function(config) {
        if (!config.iconurl) {
            ...
            if (!config.iconComponent || config.iconComponent == 'powereduc') {
