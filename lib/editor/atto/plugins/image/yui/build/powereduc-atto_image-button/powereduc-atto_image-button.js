YUI.add('powereduc-atto_image-button', function (Y, NAME) {
...
/**
 * @module powereduc-atto_image_alignment-button
 */
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _uploadImage: function(fileToSave) {
        ...
        uploadid = 'powereducimage_' + Math.round(Math.random() * 100000) + '-' + timestamp;
        host.focus();
        host.restoreSelection();
        imagehtml = template({
            url: M.util.image_url("i/loading_small", 'powereduc'),
