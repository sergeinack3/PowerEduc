...
/**
 * Tiny Media plugin Embed class for PowerEduc.
 *
...
import {getCurrentLanguage, getPowerEducLang} from 'editor_tiny/options';
...
export const MediaEmbed = class {
    ...
    async getTemplateContext(data) {
        const languages = this.preparePowerEducLang();
    ...
    preparePowerEducLang() {
        const powereducLangs = getPowerEducLang(this.editor);
        const currentLanguage = getCurrentLanguage(this.editor);

        const installed = Object.entries(powereducLangs.installed).map(([lang, code]) => ({
            lang,
            code,
            "default": lang === currentLanguage,
        }));

        const available = Object.entries(powereducLangs.available).map(([lang, code]) => ({
            lang,
            code,
            "default": lang === currentLanguage,
        }));
    ...
    getPowerEducLangObj(subtitleLang) {
        const {available} = getPowerEducLang(this.editor);
    ...
    filePickerCallback(params, element, fpType) {
        if (params.url !== '') {
            ...
            if (fpType === 'subtitle') {
                ...
                const langObj = this.getPowerEducLangObj(subtitleLang);
                if (langObj) {
