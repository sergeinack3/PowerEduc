...
const importPluginList = async(pluginList) => {
    ...
    // If a plugin contains a '/' then it is assumed to be a PowerEduc AMD module to import.
...
const getStandardConfig = (target, tinyMCE, options, plugins) => {
    ...
    const config = Object.assign({}, getDefaultConfiguration(), {
        ...
        // The UI is not ideal and we'll wait for it to improve in future before we enable it in PowerEduc.
        // eslint-disable-next-line camelcase
...
const getEditorConfiguration = (target, tinyMCE, options, pluginValues) => {
    ...
    // These are not useful for PowerEduc and we don't want to encourage their use.
