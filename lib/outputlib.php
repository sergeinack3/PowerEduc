...
/**
 * Functions for generating the HTML that PowerEduc should output.
 *
...
/**
 * This class represents the configuration variables of a PowerEduc theme.
 *
 ...
 * workings of PowerEduc's themes system. If you are just editing a themes config.php
 ...
 * However, normally you don't need to bother, because powereduc_page (that is, $PAGE)
...
class theme_config {
     ...
     * Pages in PowerEduc are using several different layouts like 'normal', 'course', 'home',
     * 'popup', 'form', .... The most reliable way to get a complete list is to look at
     * {@link http://cvs.powereduc.org/powereduc/theme/base/config.php?view=markup the base theme config.php file}.
     ...
     *
     * This is an advanced feature. PowerEduc output is generated by 'renderers',
     * you can customise the HTML that is output by writing custom renderers,
     * and then you need to specify 'renderer factory' so that PowerEduc can find
     * your renderers.
     *
     * There are some renderer factories supplied with PowerEduc. Please follow these
     ...
     *  - 'html5' the only officially supported doctype in PowerEduc
    ...
    public static function load($themename) {
        ...
        try {
            ...
            // most probably powereduc tables not created yet
        ...
        if (is_readable($rendererfile)) {
            ...
            if (is_readable($this->dir.'/renderer.php')) {
                debugging('Developer hint: '.$this->dir.'/renderer.php should be renamed to ' . $this->dir."/renderers.php.
                    See: http://docs.powereduc.org/dev/Output_renderers#Theme_renderers.", DEBUG_DEVELOPER);
            }
        }
     ...
     */
    public function init_page(powereduc_page $page) {
    ...
    public function editor_css_url($encoded=true) {
        ...
        if ($rev > -1) {
            ...
            $url = new powereduc_url("/theme/styles.php");
            ...
            $url = new powereduc_url('/theme/styles_debug.php', [
                'theme' => $this->name,
                'type' => $type,
            ]);
        }
     ...
     *
     * @param powereduc_page $page Not used... deprecated?
     * @return powereduc_url[]
     */
    public function css_urls(powereduc_page $page) {
        global $CFG;
        ...
        if ($rev > -1) {
            ...
            $baseurl = new powereduc_url('/theme/styles_debug.php');
            ...
            if (core_useragent::is_ie()) {
                ...
                $urls[] = new powereduc_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'plugins'));
                foreach ($css['parents'] as $parent=>$sheets) {
                    ...
                    $urls[] = new powereduc_url($baseurl, array('theme'=>$this->name,'type'=>'ie', 'subtype'=>'parents', 'sheet'=>$parent));
                }
                if ($this->get_scss_property()) {
                    // No need to define the type as IE here.
                    $urls[] = new powereduc_url($baseurl, array('theme' => $this->name, 'type' => 'scss'));
                }
                $urls[] = new powereduc_url($baseurl, array('theme'=>$this->name, 'type'=>'ie', 'subtype'=>'theme'));

            } else {
                foreach ($css['plugins'] as $plugin=>$unused) {
                    $urls[] = new powereduc_url($baseurl, array('theme'=>$this->name,'type'=>'plugin', 'subtype'=>$plugin));
                }
                foreach ($css['parents'] as $parent=>$sheets) {
                    foreach ($sheets as $sheet=>$unused2) {
                        $urls[] = new powereduc_url($baseurl, array('theme'=>$this->name,'type'=>'parent', 'subtype'=>$parent, 'sheet'=>$sheet));
                    }
                }
                foreach ($css['theme'] as $sheet => $filename) {
                    if ($sheet === self::SCSS_KEY) {
                        ...
                        $urls[] = new powereduc_url($baseurl, array('sheet'=>$sheet, 'theme'=>$this->name, 'type'=>'theme'));
                    }
                }
            }
        }
    ...
    public function javascript_url($inhead) {
        ...
        if (!empty($CFG->slasharguments) and $rev > 0) {
            $url = new powereduc_url("/theme/javascript.php");
            $url->set_slashargument('/'.$this->name.'/'.$rev.'/'.$params['type'], 'noparam', true);
            return $url;
        } else {
            return new powereduc_url('/theme/javascript.php', $params);
        }
    }
    ...
    public function image_url($imagename, $component) {
        ...
        $url = new powereduc_url("/theme/image.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function font_url($font, $component) {
        ...
        $url = new powereduc_url("/theme/font.php");
        if (!empty($CFG->slasharguments) and $rev > 0) {
    ...
    public function setting_file_url($setting, $filearea) {
        ...
        $url = powereduc_url::make_file_url("$CFG->wwwroot/pluginfile.php", "/$syscontext->id/$component/$filearea/$itemid".$filepath);

        // Now this is tricky because the we can not hardcode http or https here, lets use the relative link.
        // Note: unfortunately powereduc_url does not support //urls yet.
    ...
    public function resolve_image_location($image, $component, $svg = false) {
        ...
        if ($component === 'powereduc' or $component === 'core' or empty($component)) {
            ...
            // In PowerEduc 4.0 we introduced a new image format.
     ...
     * At that point the were the following image counts in PowerEduc core:
     *
    ...
    /**
     * Get the renderer for a part of PowerEduc for this theme.
     *
     * @param powereduc_page $page the page we are rendering
     * @param string $component the name of part of powereduc. E.g. 'core', 'quiz', 'qtype_multichoice'.
