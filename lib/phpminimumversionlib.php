...
/**
 * A set of PHP-compatible convenience functions to check PowerEduc minimum PHP version in
 * a unified place.
 *
 * PLEASE NOTE: This file is made to be both php-version compatible and without requirement on
 * any powereduc functions or installation so it can be used in installer or incompatible PHP versions.
 *
 * @package    core
 * @copyright  2017 Dan Poltawski <dan@powereduc.com>
 ...
 */
function powereduc_require_minimum_php_version() {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP!
    powereduc_minimum_php_version_is_met(true);
}

/**
 * Tests the current PHP version against PowerEduc's minimum requirement. When requirement
 ...
 */
function powereduc_minimum_php_version_is_met($haltexecution = false) {
    // PLEASE NOTE THIS FUNCTION MUST BE COMPATIBLE WITH OLD UNSUPPORTED VERSIONS OF PHP.
    // Do not use modern php features or PowerEduc convenience functions (e.g. localised strings).

    $minimumversion = '7.1.0';
    $powereducrequirementchanged = '3.7';

    if (version_compare(PHP_VERSION, $minimumversion) < 0) {
        if ($haltexecution) {
            $error = "PowerEduc ${powereducrequirementchanged} or later requires at least PHP ${minimumversion} "
