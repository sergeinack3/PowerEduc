...
We include these in PowerEduc solely for the convenience of sites that
may not have PEAR installed.

If this directory is DELETED from PowerEduc then PowerEduc will search
...
- PEAR HTML_Quickform:
    ...
    - License: PHP (Permission given to PowerEduc to redistribute under GPL)
...
and use wrapper classes to isolate PowerEduc code from internal PEAR
...
the old calls used before PowerEduc 1.6 to maintain compatibility.
...
Everything in PowerEduc in pure GPL.  This pear directory is the only
part of the distribution that is not.

There is some question about how PHP-licensed software can be
included within a GPL-licensed distribution like PowerEduc, specifically
the clause that annoyingly says no derivative of the software can
include the name PHP.

We don't intend to rename PowerEduc to anything of the sort, obviously,
...
Several authors have given PowerEduc explicit permission to distribute
their PHP-licensed PEAR classes in the PowerEduc distribution, allowing
anybody using these classes ONLY as part of the PowerEduc distribution
exemption from clauses of the PHP license that could cause
conflict with the main GNU Public License that PowerEduc uses.
...
this directory from PowerEduc and it will use your installed PEAR
libraries instead.
