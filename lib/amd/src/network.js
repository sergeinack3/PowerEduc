...
define(['jquery', 'core/ajax', 'core/config', 'core/notification', 'core/str'],
        ...
        return Ajax.call([request], true, true, true)[0].then(function(args) {
            if (args.timeremaining * 1000 > warningLimit) {
                ...
                return Str.get_strings([
                    ...
                    {key: 'loginagain', component: 'powereduc'},
    ...
    var checkSession = function() {
        ...
        return Ajax.call([request], true, true, true)[0].then(function(args) {
            ...
            if (args.timeremaining <= 0) {
                ...
                Str.get_strings([
                    {key: 'norecentactivity', component: 'powereduc'},
                    {key: 'sessiontimeoutsoon', component: 'powereduc'},
                    {key: 'extendsession', component: 'powereduc'},
                    {key: 'cancel', component: 'powereduc'}
                ]).then(function(strings) {
    ...
    /**
     * Are we in an iframe and the parent page is from the same PowerEduc site?
     *
     * @return {boolean} true if we are in an iframe in a page from this PowerEduc site.
     */
    const isPowerEducIframe = function() {
        if (window.parent === window) {
            // Not in an iframe.
            return false;
        }

        // We are in an iframe. Is the parent from the same PowerEduc site?
        let parentUrl;
        try {
    ...
    var init = function() {
        ...
        if (isPowerEducIframe()) {
            window.console.log('Not starting PowerEduc session timeout warning in this iframe.');
            return;
        }

        window.console.log('Starting PowerEduc session timeout warning.');
    ...
    var keepalive = function(freq, timeout, message) {
        ...
        if (isPowerEducIframe()) {
            window.console.warn('Ignoring session keep-alive in this iframe inside another PowerEduc page.');
            return;
        }

        window.console.log('Starting PowerEduc session keep-alive.');
