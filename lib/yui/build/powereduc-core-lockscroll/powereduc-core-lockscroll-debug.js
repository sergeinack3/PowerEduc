YUI.add('powereduc-core-lockscroll', function (Y, NAME) {

/**
 * Provides the ability to lock the scroll for a page, allowing nested
 * locking.
 *
 * @module powereduc-core-lockscroll
 */
...
Y.namespace('M.core').LockScroll = Y.Base.create('lockScroll', Y.Plugin.Base, [], {
    ...
    enableScrollLock: function(forceOnSmallWindow) {
        if (this.isActive()) {
            Y.log('LockScroll already active. Ignoring enable request', 'warn', 'powereduc-core-lockscroll');
            return;
        }

        if (!this.shouldLockScroll(forceOnSmallWindow)) {
            Y.log('Dialogue height greater than window height. Ignoring enable request.', 'warn', 'powereduc-core-lockscroll');
            return;
        }

        Y.log('Enabling LockScroll.', 'debug', 'powereduc-core-lockscroll');
        this._enabled = true;
        ...
        Y.log("Setting the activeScrollLocks count from " + currentCount + " to " + newCount,
                'debug', 'powereduc-core-lockscroll');
    ...
    disableScrollLock: function(force) {
        if (this.isActive()) {
            Y.log('Disabling LockScroll.', 'debug', 'powereduc-core-lockscroll');
            this._enabled = false;
