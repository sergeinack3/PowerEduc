YUI.add('powereduc-core-notification-exception', function (Y, NAME) {
 ...
 * @submodule powereduc-core-notification-exception
 */

var EXCEPTION_NAME = 'PowerEduc exception',
    EXCEPTION;
...
Y.extend(EXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h5 id="powereduc-dialogue-' + this.get('COUNT') + '-wrap-header-text">' + Y.Escape.html(config.name) + '</h5>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="powereduc-exception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="powereduc-exception-message">' + Y.Escape.html(this.get('message')) + '</div>'))
                .append(Y.Node.create('<div class="powereduc-exception-param hidden param-filename"><label>' +
                        M.util.get_string('file', 'powereduc') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('fileName')) + '</div>'))
                .append(Y.Node.create('<div class="powereduc-exception-param hidden param-linenumber"><label>' +
