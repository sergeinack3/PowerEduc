YUI.add('powereduc-core-notification-ajaxexception', function (Y, NAME) {
 ...
 * thrown in the PowerEduc codebase was reported during an AJAX request.
 *
 * @module powereduc-core-notification
 * @submodule powereduc-core-notification-ajaxexception
 */

var AJAXEXCEPTION_NAME = 'PowerEduc AJAX exception',
    AJAXEXCEPTION;
...
AJAXEXCEPTION = function(config) {
    config.name = config.name || M.util.get_string('error', 'powereduc');
...
Y.extend(AJAXEXCEPTION, M.core.notification.info, {
    ...
    initializer: function(config) {
        ...
        this.get(BASE).addClass('powereduc-dialogue-exception');
        this.setStdModContent(Y.WidgetStdMod.HEADER,
                '<h5 id="powereduc-dialogue-' + this.get('COUNT') + '-wrap-header-text">'
                    + Y.Escape.html(config.name) + '</h5>',
                Y.WidgetStdMod.REPLACE);
        content = Y.Node.create('<div class="powereduc-ajaxexception" data-rel="fatalerror"></div>')
                .append(Y.Node.create('<div class="powereduc-exception-message">' + Y.Escape.html(this.get('error')) + '</div>'))
                .append(Y.Node.create('<div class="powereduc-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('url', 'powereduc') + labelsep + '</label> ' +
                        this.get('reproductionlink') + '</div>'))
                .append(Y.Node.create('<div class="powereduc-exception-param hidden param-debuginfo"><label>' +
                        M.util.get_string('debuginfo', 'debug') + labelsep + '</label> ' +
                        Y.Escape.html(this.get('debuginfo')) + '</div>'))
                .append(Y.Node.create('<div class="powereduc-exception-param hidden param-stacktrace"><label>' +
                        M.util.get_string('stacktrace', 'debug') + labelsep + '</label> <pre>' +
                        Y.Escape.html(this.get('stacktrace')) + '</pre></div>'));
        if (M.cfg.developerdebug) {
            content.all('.powereduc-exception-param').removeClass('hidden');
        }
    ...
    ATTRS: {
        ...
        error: {
            ...
            value: M.util.get_string('unknownerror', 'powereduc')
        },
