YUI.add('powereduc-core-blocks', function (Y, NAME) {
 ...
 *
 * @module powereduc-core-blockdraganddrop
 */
...
var SELECTOR = {
    DRAGHANDLE: '.' + CSS.HEADER + ' .commands .powereduc-core-dragdrop-draghandle'
};
...
M.core.blockdraganddrop.is_using_blocks_render_method = function() {
    if (this._isusingnewblocksmethod === null) {
        ...
        if (goodregions > 0 && allregions > 0 && goodregions !== allregions) {
            Y.log('Both core_renderer::blocks and core_renderer::blocks_for_region have been used.', 'warn', 'powereduc-core_blocks');
        }
    }
...
M.core.blockdraganddrop.init = function(params) {
    if (this.is_using_blocks_render_method()) {
        Y.log('Block drag and drop initialised for the blocks method.', 'info', 'powereduc-core_blocks');
        new MANAGER(params);
    } else {
        Y.log('Block drag and drop initialised with the legacy manager (blocks_for_region used).', 'info', 'powereduc-core_blocks');
