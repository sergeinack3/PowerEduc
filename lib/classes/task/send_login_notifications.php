 ...
 *
 * @package    core
 * @copyright  2021 PowerEduc Pty Ltd.
 * @author     Juan Leyva <juan@powereduc.com>
...
class send_login_notifications extends adhoc_task {
    ...
    public function execute() {
        ...
        if (!empty($customdata->ispowereducapp)) {
        ...
        $logindevice = $customdata->ispowereducapp ? get_string('mobileapp', 'tool_mobile') : '';
        $logindevice .= ' ' . $customdata->useragent;
        $loginip = $customdata->loginip;
        $logintime = userdate($customdata->logintime);

        $changepasswordlink = (new \powereduc_url('/user/preferences.php', ['userid' => $USER->id]))->out(false);
        ...
        if ($userauth->can_change_password()) {
            if ($changepwurl = $userauth->change_password_url()) {
                ...
                $changepasswordlink = (new \powereduc_url('/login/change_password.php'))->out(false);
            }
        }
        ...
        $eventdata->subject           = get_string('newloginnotificationtitle', 'powereduc', $sitename);
        $eventdata->fullmessageformat = FORMAT_HTML;
        $info = compact('sitename', 'siteurl', 'userfullname', 'username', 'useremail',
            'logindevice', 'logintime', 'loginip', 'changepasswordlink');
        $eventdata->fullmessagehtml   = get_string('newloginnotificationbodyfull', 'powereduc', $info);
        $eventdata->fullmessage       = html_to_text($eventdata->fullmessagehtml);
        $eventdata->smallmessage      = get_string('newloginnotificationbodysmall', 'powereduc', $username);
