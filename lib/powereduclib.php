...
/**
 * PARAM_CAPABILITY - A capability name, like 'powereduc/role:manage'. Actually
...
/**
 * PAGE_COURSE_VIEW is a definition of a page type. For more information on the page class see powereduc/lib/pagelib.php.
 */
...
/** True if module supports backup/restore of powereduc2 format */
define('FEATURE_BACKUP_POWEREDUC2', 'backup_powereduc2');
...
function required_param($parname, $type) {
    ...
    if (isset($_POST[$parname])) {
        ...
        throw new \powereduc_exception('missingparam', '', '', $parname);
    }
...
function clean_param($param, $type) {
    ...
    switch ($type) {
        ...
        default:
            ...
            throw new \powereduc_exception("unknownparamtype", '', '', $type);
    }
}
...
function get_config($plugin, $name = null) {
    ...
    if ($plugin === 'powereduc' || $plugin === 'core' || empty($plugin)) {
 ...
 * @param    stdClass|int|null $user  A powereduc user object or id, null means current user
 * @throws   coding_exception
 ...
 *             if 99 then default user's timezone is used {@link http://docs.powereduc.org/dev/Time_API#Timezone}
...
function require_login($courseorid = null, $autologinguest = true, $cm = null, $setwantsurltome = true, $preventredirect = false) {
    ...
    if ($course->id != SITEID and \core\session\manager::is_loggedinas()) {
        if ($USER->loginascontext->contextlevel == CONTEXT_COURSE) {
            if ($USER->loginascontext->instanceid != $course->id) {
                throw new \powereduc_exception('loginasonecourse', '',
    ...
    if (get_user_preferences('auth_forcepasswordchange') && !\core\session\manager::is_loggedinas()) {
        ...
        if ($userauth->can_change_password() and !$preventredirect) {
            ...
            if ($changeurl = $userauth->change_password_url()) {
                ...
                // Use powereduc internal method.
                redirect($CFG->wwwroot .'/login/change_password.php');
            }
        } else if ($userauth->can_change_password()) {
            throw new powereduc_exception('forcepasswordchangenotice');
        } else {
            throw new powereduc_exception('nopasswordchangeforced', 'auth');
        }
    }
    ...
    if ($usernotfullysetup) {
        if ($preventredirect) {
            throw new powereduc_exception('usernotfullysetup');
        }
        if ($setwantsurltome) {
            $SESSION->wantsurl = qualified_me();
        }
    ...
    if (!$USER->policyagreed && !is_siteadmin() && !NO_SITEPOLICY_CHECK) {
        ...
        if ($policyurl = $manager->get_redirect_url(isguestuser())) {
            if ($preventredirect) {
                throw new powereduc_exception('sitepolicynotagreed', 'error', '', $policyurl->out());
            }
            if ($setwantsurltome) {
                $SESSION->wantsurl = qualified_me();
            }
    ...
    if (!empty($CFG->maintenance_enabled) and !has_capability('powereduc/site:maintenanceaccess', $sysctx)) {
        if ($preventredirect) {
    ...
    if ($cm && $cm->deletioninprogress) {
        if ($preventredirect) {
            throw new powereduc_exception('activityisscheduledfordeletion');
        }
...
function validate_user_key($keyvalue, $script, $instance) {
    ...
    if (!empty($key->validuntil) and $key->validuntil < time()) {
        throw new \powereduc_exception('expiredkey');
    }

    if ($key->iprestriction) {
        $remoteaddr = getremoteaddr(null);
        if (empty($remoteaddr) or !address_in_subnet($remoteaddr, $key->iprestriction)) {
            throw new \powereduc_exception('ipmismatch');
        }
    }
    return $key;
}
...
function require_user_key_login($script, $instance = null, $keyvalue = null) {
    ...
    if (!NO_POWEREDUC_COOKIES) {
        throw new \powereduc_exception('sessioncookiesdisable');
    }
...
function get_auth_plugin($auth) {
    ...
    if (! exists_auth_plugin($auth)) {
        throw new \powereduc_exception('authpluginnotfound', 'debug', '', $auth);
    }
        ...
        if (!empty($CFG->passwordpolicycheckonlogin)) {
            ...
            if (!$passed) {
                ...
                if ($authplugin->can_change_password()) {
                    ...
                    $internalchangeurl = new powereduc_url('/login/change_password.php');
                    ...
                    redirect(new powereduc_url('/login/forgot_password.php'));
                } else {
...
function complete_user_login($user, array $extrauserinfo = []) {
    ...
    $isvalidenv = (!WS_SERVER && !CLI_SCRIPT && !NO_POWEREDUC_COOKIES) || PHPUNIT_TEST;

    if (!empty($SESSION->isnewsessioncookie) && $isnewip && $isvalidenv && !\core_useragent::is_powereduc_app()) {

        $logintime = time();
        $ispowereducapp = false;
 ...
 *
 * @throws powereduc_exception If a problem occurs while generating the hash.
 */
function hash_internal_user_password($password, $fasthash = false) {
    ...
    if ($generatedhash === false || $generatedhash === null) {
        throw new powereduc_exception('Failed to generate password hash.');
    }
 ...
 */
function powereduc_process_email($modargs, $body) {
    global $DB;
 ...
 * @return powereduc_phpmailer|null mailer instance if 'get' used or nothing
 */
function get_mailer($action='get') {
    global $CFG;

    /** @var powereduc_phpmailer $mailer */
    static $mailer  = null;
    static $counter = 0;
...
/**
 * Generate a unique email Message-ID using the powereduc domain and install path
 *
...
function generate_email_messageid($localpart = null) {
    ...
    // If multiple powereducs are on the same domain we want to tell them
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
    ...
    // header with details of where exactly in powereduc the email was triggered from,
    ...
    if ($mail->send()) {
        ...
        $event->trigger();
        if (CLI_SCRIPT) {
            mtrace('Error: lib/powereduclib.php email_to_user(): '.$mail->ErrorInfo);
        }
        if (!empty($mail->SMTPDebug)) {
            echo '</pre>';
        }
...
function reset_password_and_mail($user) {
    ...
    if (!$userauth->user_update_password($user, $newpassword)) {
        throw new \powereduc_exception("cannotsetpassword");
    }
...
function send_confirmation_email($user, $confirmationurl = null) {
    ...
    $confirmationurl = new powereduc_url($confirmationurl);
    ...
    // Because of this custom encoding we can't use powereduc_url directly.
...
function get_file_packer($mimetype='application/zip') {
    ...
    switch ($mimetype) {
        ...
        case 'application/vnd.powereduc.profiling':
        ...
        case 'application/vnd.powereduc.backup':
...
function get_user_max_upload_file_size($context, $sitebytes = 0, $coursebytes = 0, $modulebytes = 0, $user = null,
    ...
    if (has_capability('powereduc/course:ignorefilesizelimits', $context, $user)) {
 ...
 * to retrieve it.<br/>
 * If you open powereduc/lang/en/powereduc.php and look near line 278
 ...
 * different approach. Looking in powereduc/lang/en/calendar.php you find
 * around line 75:
 * <code>
 ...
 *      If none is specified then powereduc.php is used.
 ...
 *
 * Example usage of this function when the string is in the powereduc.php file:<br/>
 * <code>
 * echo '<strong>';
 * print_string('course');
 * echo '</strong>';
 * </code>
 *
 * Example usage of this function when the string is not in the powereduc.php file:<br/>
 * <code>
 * echo '<h1>';
 ...
 * @category string
 * @param string $identifier The key identifier for the localized string
 * @param string $component The module where the key identifier is stored. If none is specified then powereduc.php is used.
...
/**
 * Determine if powereduc installation requires update.
 *
 ...
 * @return bool
 */
function powereduc_needs_upgrading() {
    global $CFG;
 ...
 */
function powereduc_major_version($fromdisk = false) {
    global $CFG;
 ...
 */
function powereduc_setlocale($locale='') {
    global $CFG;
    ...
    if (!empty($locale)) {
        ...
        $currentlocale = powereduc_getlocale();
    }
 ...
 * replace an existing powereduc script
 *
...
function set_mnet_remote_client($client) {
    if (!defined('MNET_SERVER')) {
        throw new powereduc_exception('notinxmlrpcserver', 'mnet');
    }
    global $MNET_REMOTE_CLIENT;
 ...
 *     $string = get_string('yes', 'powereduc', null, true);
