YUI.add('powereduc-form-shortforms', function (Y, NAME) {

/**
 * Provides the form shortforms class.
 *
 * @module powereduc-form-shortforms
 */
...
Y.extend(SHORTFORMS, Y.Base, {
    ...
    initializer: function() {
        ...
        if (!form) {
            Y.log('Could not locate the form', 'warn', 'powereduc-form-shortforms');
            return;
        }
    ...
    set_state: function(fieldset, collapsed) {
        ...
        if (!statuselement) {
            Y.log("M.form.shortforms::switch_state was called on an fieldset without a status field: '" +
                fieldset.get('id') + "'", 'debug', 'powereduc-form-shortforms');
            return this;
        }
    ...
    NAME: 'powereduc-form-shortforms',
    ATTRS: ATTRS
});
...
}, '@VERSION@', {"requires": ["node", "base", "selector-css3", "powereduc-core-event"]});
