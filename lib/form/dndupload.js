...
M.form_dndupload.init = function(Y, options) {
    var dnduploadhelper = {
        ...
        update_progress: function(filename, percent) {
            ...
            this.progressbars[filename].progressinnertext.setContent(percent + '% ' + M.util.get_string('complete', 'powereduc'));
        },
    ...
    Y.extend(dnduploader, Y.Base, {
        ...
        print_msg: function(msg, type) {
            var header = M.util.get_string('error', 'powereduc');
            if (type != 'error') {
                type = 'info'; // one of only two types excepted
                header = M.util.get_string('info', 'powereduc');
            }
            if (!this.msg_dlg) {
        ...
        display_size: function(size) {
            // This is snippet of code (with some changes) is from the display_size function in powereduclib.
            var gb = M.util.get_string('sizegb', 'powereduc'),
                mb = M.util.get_string('sizemb', 'powereduc'),
                kb = M.util.get_string('sizekb', 'powereduc'),
                b  = M.util.get_string('sizeb', 'powereduc');
        ...
        add_to_upload_queue: function(file, filename, overwrite) {
            ...
            if (this.options.maxfiles >= 0 && this.getTotalNumberOfFiles() > this.options.maxfiles) {
                ...
                this.uploadqueue = [];
                this.renamequeue = [];
                this.print_msg(M.util.get_string('maxfilesreached', 'powereduc', this.options.maxfiles), 'error');
                return false;
            }
            ...
            if (this.options.areamaxbytes > -1) {
                ...
                if (sizereached > this.options.areamaxbytes) {
                    this.uploadqueue = [];
                    this.renamequeue = [];
                    this.print_msg(M.util.get_string('maxareabytesreached', 'powereduc'), 'error');
                    return false;
                }
            }
        ...
        process_renames: function() {
            ...
            node.one('.fp-dlg-text').setContent(M.util.get_string('fileexists', 'powereduc', file.name));
