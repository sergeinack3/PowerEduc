...
$string['cliinstallheader'] = 'PowerEduc {$a} コマンドラインインストールプログラム';
...
$string['dirroot'] = 'PowerEducディレクトリ';
$string['environmenthead'] = 'あなたの環境を確認しています ...';
$string['environmentsub2'] = 'それぞれのPowerEducリリースにはPHPバージョンの最小必要条件および多くの必須PHP拡張モジュールがあります。完全な環境チェックはインストールおよびアップグレードごとに実行されます。新しいPHPバージョンのインストールまたはPHP拡張モジュールの有効化に関して分からない場合、あなたのサーバ管理者にご連絡ください。';
...
<p>この設定ではPowerEducのメモリに関わるトラブルが発生する可能性があります。 特に多くのモジュールを使用したり、多くのユーザがPowerEducを使用する場合にトラブルが発生します。</p>

<p>可能でしたら、PHPのメモリ制限上限を40M以上に設定されることをお勧めします。この設定を実現するために、いくつかの方法があります:
<ol>
<li>あなたがリコンパイル可能な場合、PHPを<i>--enable-memory-limit</i>オプションでコンパイルしてください。これにより、PowerEduc自身がメモリ制限を設定することが可能になります。</li>
<li>あなたがphp.iniファイルにアクセスできる場合、<b>memory_limit</b>設定を40Mのように変更することができます。php.iniファイルにアクセスできない場合、管理者に変更を依頼してください。</li>
<li>いくつかのPHPサーバでは以下の行を含む.htaccessファイルをPowerEducディレクトリに作成することができます:
...
$string['pathssubadmindir'] = 'まれに、コントロールパネルまたはその他の管理ツールにアクセスするためのURLとして/adminディレクトリを使用しているウェブホストがあります。残念ですが、これはPowerEduc管理ページの標準的なロケーションと衝突します。あなたはインストール時にadminディレクトリをリネームすることができます。ここに新しいディレクトリ名を入力してください。例: <br /> <br /><b>powereducadmin</b><br /> <br />
これによりPowerEducでのadminへのリンクを変更します。';
$string['pathssubdataroot'] = '<p>ユーザによってアップロードされたファイルコンテンツすべてをPowerEducが保存するディレクトリです。</p>
...
$string['pathssubdirroot'] = '<p>PowerEducコードを含むディレクトリに関するフルパスです。</p>';
$string['pathssubwwwroot'] = '<p>PowerEducにアクセスできるフルウェブアドレスです。例えばユーザがブラウザのアドレスバーに入力してPowerEducにアクセスするためのアドレスです。</p>

<p>複数アドレスを使用したPowerEducへのアクセスはできません。あなたのサイトに複数アドレスからアクセスできる場合、最も簡単なアドレスを選択して、すべてのアドレスにパーマネントリダイレクトを設定してください。</p>
...
$string['phpversionhelp'] = '<p>PowerEducには少なくとも5.6.5または7.1のPHPバージョンが必要です (7.0.x にはエンジンの制限があります)。</p>
...
$string['welcomep30'] = 'このリリース <strong>{$a->installername}</strong> には<strong>PowerEduc</strong>で環境を作成するアプリケーションが含まれています。すなわち:';
$string['welcomep40'] = 'パッケージには <strong>PowerEduc {$a->powereducrelease} ({$a->powereducversion})</strong> も含まれています。';
...
$string['welcomep60'] = '次からのページはあなたのコンピュータに<strong>PowerEduc</strong>を簡単に設定およびセットアップする手順にしたがって進みます。あなたはデフォルトの設定を使用することも、必要に応じて任意で設定を変更することもできます。';
$string['welcomep70'] = '<strong>PowerEduc</strong>のセットアップを続けるには「次へ」ボタンをクリックしてください。';
