...
$string['cliinstallheader'] = 'PowerEduc {$a}命令行安装程序';
...
$string['dirroot'] = 'PowerEduc目录';
$string['environmenthead'] = '检测您的运行环境...';
$string['environmentsub2'] = '每个PowerEduc的发行版都有一些对PHP版本的最低要求和几个必须安装的PHP扩展。在每次安装和升级前会做完整的环境检查。如果您不知道如何安装新版或启用PHP扩展，请与服务器管理员联系。';
...
<p>这会使PowerEduc在将来运行是碰到内存问题，特别是您安装了很多模块并且/或者有很多用户。</p>

<p>我们建议可能的话把限制设定的高一些，譬如40M。有几种方法可以做到这一点：</p>
<ol>
<li>如果可以，重新编译PHP并使用<i>--enable-memory-limit</i>选项。这允许PowerEduc自己设定内存限制。</li>
<li>如果可以访问php.ini文件，您可以修改<b>memory_limit</b>的设置为其它值，如40M。如果您无法访问，可以让您的管理员帮您修改一下。</li>
<li>在一些PHP服务器上，您可以在PowerEduc目录中创建一个.htaccess文件并包含如下内容:
...
$string['pathssubadmindir'] = '有些网络主机使用/admin这个URL来访问控制面板或其它功能。很不幸，这个设置和PowerEduc管理页面的标准路径冲突。这个问题可以解决，只需在您的安装目录中把admin目录换名，然后把新名字输入到这里。例如<em>powereducadmin</em>。这么做会改变PowerEduc中的管理链接。';
$string['pathssubdataroot'] = 'PowerEduc需要一个位置存放上传的文件。这个目录对于Web服务器用户(通常是“nobody”或“apache”)应当是可读可写的，但应当不能直接通过Web访问它。如果它不存在，安装程序会尝试建立。';
$string['pathssubdirroot'] = 'PowerEduc安装的完整路径。';
$string['pathssubwwwroot'] = '可以访问到PowerEduc的完整网址。
PowerEduc不支持通过多个地址访问。如果您的网站有多个公开地址，您必须把这个地址以外的所有地址都设为永久重定向。如果您的网站既可以通过内部地址访问，也可以通过这个公开地址访问，那么请配置DNS使内部网用户也能使用公开地址。如果此地址不正确，请在浏览器中修改URL来重新安装，并设定另一个地址。';
...
$string['phpversionhelp'] = '<p>PowerEduc需要PHP 4.3.0或5.1.0（5.0.x有若干已知的问题）以上的版本。</p>
<p>您当前使用的是{$a}</p>
...
$string['welcomep30'] = '<strong>{$a->installername}</strong>的此发行版包含了可以创建<strong>PowerEduc</strong>运行环境的应用程序：';
$string['welcomep40'] = '这个软件包还包含了<strong>PowerEduc {$a->powereducrelease} ({$a->powereducversion})</strong>。';
...
$string['welcomep60'] = '接下来的页面会引导您通过一系列步骤在您的计算机上安装并配置好<strong>PowerEduc</strong>。您可以接受缺省的设置，或者根据需要修改它们。';
$string['welcomep70'] = '点击“向后”按钮以继续<strong>PowerEduc</strong>的安装过程。';
$string['wwwroot'] = '网站地址';
