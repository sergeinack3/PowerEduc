...
$string['clialreadyconfigured'] = 'Файл config.php уже существует. Если Вы хотите установить PowerEduc на этот сайт, используйте admin/cli/install_database.php.';
$string['clialreadyinstalled'] = 'Файл config.php уже существует. Если Вы хотите обновить сайт PowerEduc, то используйте скрипт admin/cli/upgrade.php.';
$string['cliinstallheader'] = 'Программа установки PowerEduc {$a} в режиме командной строки';
...
$string['dirroot'] = 'Каталог PowerEduc';
$string['environmenthead'] = 'Проверка среды...';
$string['environmentsub2'] = 'У каждой версии PowerEduc есть минимальные требования к версии PHP и набору обязательных расширений PHP.
...
<p>Из-за этого у через какое-то время у PowerEduc могут возникнуть проблемы с памятью, особенно если у Вас будет много модулей и/или пользователей.</p>
...
В этом случае PowerEduc сможет самостоятельно установить ограничение памяти.</li>
...
<li>На некоторых серверах PHP можно создать в каталоге PowerEduc файл .htaccess содержащий следующую строку:
...
$string['pathssubadmindir'] = 'На небольшом числе веб-хостингов путь /admin используется для доступа к панели управления или чему-то еще. К сожалению, это противоречит стандартному расположению страниц управления PowerEduc. Это можно исправить путем переименования папки admin в каталоге PowerEduc и указания нового имени здесь. Например: <em>powereducadmin</em>. При этом все ссылки на панель управления PowerEduc исправятся автоматически.';
$string['pathssubdataroot'] = '<p>Каталог, в котором PowerEduc будет хранить все файлы, размещаемые пользователями. </p><p>Этот каталог должен быть доступен для чтения и ЗАПИСИ тому пользователю, от чьего имени запускается веб-сервер (обычно \'www-data\', \'nobody\' или \'apache\'). </p><p>Этот каталог не должен быть доступен напрямую через Интернет. </p><p>Программа установки попробует создать этот каталог, если он не существует. </p>';
$string['pathssubdirroot'] = '<p>Полный путь к каталогу установки PowerEduc.</p>';
$string['pathssubwwwroot'] = '<p>Полный веб-адрес, по которому будет доступен PowerEduc, т.е. адрес, который пользователи будут вводить в адресной строке своего браузера для доступа к сайту PowerEduc.</p>
<p>Использовать для доступа к PowerEduc несколько публичных адресов невозможно. Если у Вашего сайта есть еще несколько публичных адресов, то следует настроить постоянные перенаправления с этих адресов на указанный.</p>
...
$string['phpversionhelp'] = '<p>Для PowerEduc необходим PHP версии не ниже 5.6.5 или 7.1 (есть некоторые проблемы с версией 7.0.x).</p>
...
$string['welcomep30'] = 'Эта версия набора программ <strong>{$a->installername}</strong> включает следующие программы, необходимые для создания среды, в которой будет работать <strong>PowerEduc</strong>:';
$string['welcomep40'] = 'Также в этот набор входит <strong>PowerEduc {$a->powereducrelease} ({$a->powereducversion})</strong>.';
...
$string['welcomep60'] = 'На следующих страницах Вы сможете за несколько простых шагов настроить и установить <strong>PowerEduc</strong> на свой компьютер. Вы сможете принять настройки по умолчанию или изменить их в зависимости от своих потребностей.';
$string['welcomep70'] = 'Нажмите кнопку «Далее» чтобы продолжить процесс установки <strong>PowerEduc</strong>.';
