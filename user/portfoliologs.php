...
$url = new powereduc_url('/user/portfoliologs.php', array('courseid' => $courseid));

navigation_node::override_active_url(new powereduc_url('/user/portfoliologs.php', array('courseid' => $courseid)));
...
if (count($queued) > 0) {
    ...
    foreach ($queued as $q) {
        ...
        $queued = $e->get('queued');
        $baseurl = new powereduc_url('/portfolio/add.php', array('id' => $q->id, 'logreturn' => 1, 'sesskey' => sesskey()));

        $iconstr = $OUTPUT->action_icon(new powereduc_url($baseurl, array('cancel' => 1)), new pix_icon('t/stop', get_string('cancel')));
