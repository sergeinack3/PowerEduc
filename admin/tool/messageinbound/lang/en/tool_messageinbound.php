...
$string['configmessageinboundhost'] = 'The address of the server that PowerEduc should check mail against. To specify a non-default port, use [server]:[port], for example mail.example.com:993. If a port isn\'t specified, the default port for the type of mail server will be used.';
...
$string['incomingmailserversettings_desc'] = 'PowerEduc is capable of connecting to appropriately configured IMAP servers. You can specify the settings used to connect to your IMAP server here.';
...
$string['messageinboundgeneralconfiguration_desc'] = 'Inbound message processing allows you to receive and process email within PowerEduc. This has applications such as sending email replies to forum posts or adding files to a user\'s private files.';
...
$string['messageinboundhostssl_desc'] = 'Some mail servers support an additional level of security by encrypting communication between PowerEduc and your server. We recommend using this SSL encryption if your server supports it.';
...
$string['validateaddress_help'] = 'When a message is received from a user, PowerEduc attempts to validate the message by comparing the email address of the sender with the email address in their user profile.
