...
Feature: show only differences between roles for selected capabilities
  ...
  Scenario: Compare identical roles
    When I set the following fields to these values:
      | Capability: | powereduc/course:changefullname, powereduc/course:changeshortname, powereduc/course:changeidnumber, powereduc/course:changesummary |
  ...
  Scenario: Compare different roles
    ...
    And I should see "powereduc/course:changefullname" in the "comparisontable" "table"
    And I should see "powereduc/course:changeshortname" in the "comparisontable" "table"
    And I should not see "powereduc/course:changesummary" in the "comparisontable" "table"

  Scenario: Compare different roles but comparing capabilities that are equals on both
    When I set the following fields to these values:
      | Capability: | powereduc/course:changeidnumber, powereduc/course:changesummary |
      | Roles:      | Student, Studentdf                                        |
    And I set the field "Show differences only" to "1"
    And I click on "Get the overview" "button"
    Then I should see "There are no differences to show between selected roles in this context"
  ...
  Scenario: Compare all roles without selecting specific role
    When I set the following fields to these values:
      | Capability: | powereduc/course:changefullname, powereduc/site:config |
    And I set the field "Show differences only" to "1"
    And I click on "Get the overview" "button"
    Then I should not see "powereduc/site:config" in the "comparisontable" "table"
    And I should see "powereduc/course:changefullname" in the "comparisontable" "table"

  Scenario: Compare all roles without selecting specific role on not defined capability
    When I set the following fields to these values:
      | Capability: | powereduc/site:config |
    And I set the field "Show differences only" to "1"
    And I click on "Get the overview" "button"
    Then I should see "There are no differences to show between selected roles in this context"
