...
The PowerEduc UI component Library is built with the aim to provide a single repository
of PowerEduc UI components to be used for new and existing PowerEduc pages.

A PowerEduc UI component is a self-contained UI element originating from one source file
that can be used in different locations in core code, for example a button.

This plugin generates the documentation of the PowerEduc User Interface components. It
...
The PowerEduc grunt tasks were setup to include the node modules required for building a static site using
...
all the build tasks for the library are included in PowerEduc's core grunt tasks
