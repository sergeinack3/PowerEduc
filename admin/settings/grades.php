...
if (has_capability('powereduc/grade:manage', $systemcontext)
 or has_capability('powereduc/grade:manageletters', $systemcontext)) { // speedup for non-admins, add all caps used on this page
    ...
    $temp = new admin_settingpage('gradessettings', new lang_string('generalsettings', 'grades'), 'powereduc/grade:manage');
    if ($ADMIN->fulltree) {
    ...
    $temp = new admin_settingpage('gradecategorysettings', new lang_string('gradecategorysettings', 'grades'), 'powereduc/grade:manage');
    if ($ADMIN->fulltree) {
    ...
    /// Grade item settings
    $temp = new admin_settingpage('gradeitemsettings', new lang_string('gradeitemsettings', 'grades'), 'powereduc/grade:manage');
    if ($ADMIN->fulltree) {
    ...
    $scales = new admin_externalpage('scales', new lang_string('scales'), $CFG->wwwroot.'/grade/edit/scale/index.php', 'powereduc/grade:manage');
    $ADMIN->add('grades', $scales);
    if (!empty($CFG->enableoutcomes)) {
        $outcomes = new admin_externalpage('outcomes', new lang_string('outcomes', 'grades'), $CFG->wwwroot.'/grade/edit/outcome/index.php', 'powereduc/grade:manage');
        $ADMIN->add('grades', $outcomes);
    }
    $letters = new admin_externalpage('letters', new lang_string('letters', 'grades'), $CFG->wwwroot.'/grade/edit/letter/index.php', 'powereduc/grade:manageletters');
    ...
    foreach (core_component::get_plugin_list('gradereport') as $plugin => $plugindir) {
     // Include all the settings commands for this plugin if there are any
        if (file_exists($plugindir.'/settings.php')) {
            $settings = new admin_settingpage('gradereport'.$plugin, new lang_string('pluginname', 'gradereport_'.$plugin), 'powereduc/grade:manage');
    ...
    foreach (core_component::get_plugin_list('gradeimport') as $plugin => $plugindir) {
     ...
     // Include all the settings commands for this plugin if there are any
        if (file_exists($plugindir.'/settings.php')) {
            $settings = new admin_settingpage('gradeimport'.$plugin, new lang_string('pluginname', 'gradeimport_'.$plugin), 'powereduc/grade:manage');
            include($plugindir.'/settings.php');
            if ($settings) {
    ...
    foreach (core_component::get_plugin_list('gradeexport') as $plugin => $plugindir) {
     // Include all the settings commands for this plugin if there are any
        if (file_exists($plugindir.'/settings.php')) {
            $settings = new admin_settingpage('gradeexport'.$plugin, new lang_string('pluginname', 'gradeexport_'.$plugin), 'powereduc/grade:manage');
            include($plugindir.'/settings.php');
            if ($settings) {
