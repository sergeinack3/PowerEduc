...
$string['auth_shib_contact_administrator'] = 'In case you are not associated with the given organizations and you need access to a course on this server, please contact the <a href="mailto:{$a}">PowerEduc Administrator</a>.';
...
$string['auth_shibboleth_login_long'] = 'Login to PowerEduc via Shibboleth';
...
$string['auth_shib_idp_list_description'] = 'Provide a list of Identity Provider entityIDs to let the user choose from on the login page.<br />On each line there must be a comma-separated tuple for entityID of the IdP (see the Shibboleth metadata file) and Name of IdP as it shall be displayed in the drop-down list.<br />As an optional third parameter you can add the location of a Shibboleth session initiator that shall be used in case your PowerEduc installation is part of a multi federation setup.';
...
$string['auth_shib_integrated_wayf'] = 'PowerEduc WAYF service';
$string['auth_shib_integrated_wayf_description'] = 'If you enable this, PowerEduc will use its own WAYF service instead of the one configured for Shibboleth. PowerEduc will display a drop-down list on this alternative login page where the user has to select his Identity Provider.';
$string['auth_shib_logout_return_url'] = 'Alternative logout return URL';
$string['auth_shib_logout_return_url_description'] = 'Provide the URL that Shibboleth users shall be redirected to after logging out.<br />If left empty, users will be redirected to the location that powereduc will redirect users to';
...
$string['auth_shib_username_description'] = 'Name of the webserver Shibboleth environment variable that shall be used as PowerEduc username';
$string['shib_invalid_account_error'] = 'You seem to be Shibboleth authenticated but PowerEduc has no valid account for your username. Your account may not exist or it may have been suspended.';
$string['shib_no_attributes_error'] = 'You seem to be Shibboleth authenticated but PowerEduc didn\'t receive any user attributes. Please check that your Identity Provider releases the necessary attributes ({$a}) to the Service Provider PowerEduc is running on or inform the webmaster of this server.';
$string['shib_not_all_attributes_error'] = 'PowerEduc needs certain Shibboleth attributes which are not present in your case. The attributes are: {$a}<br />Please contact the webmaster of this server or your Identity Provider.';
$string['shib_not_set_up_error'] = 'Shibboleth authentication doesn\'t seem to be set up correctly because no Shibboleth environment variables are present for this page. Please consult the <a href="{$a}">README</a> for further instructions on how to set up Shibboleth authentication or contact the webmaster of this PowerEduc installation.';
