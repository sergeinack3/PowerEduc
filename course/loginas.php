...
$url = new powereduc_url('/course/loginas.php', array('id'=>$id));
$PAGE->set_url($url);
...
if (\core\session\manager::is_loggedinas()) {
    ...
    redirect(new powereduc_url($url, array('redirect'=>1)));
}
...
if (has_capability('powereduc/user:loginas', $systemcontext)) {
    if (is_siteadmin($userid)) {
        throw new \powereduc_exception('nologinas');
    }
    $context = $systemcontext;
    $PAGE->set_context($context);
} else {
    require_login($course);
    require_capability('powereduc/user:loginas', $coursecontext);
    if (is_siteadmin($userid)) {
        throw new \powereduc_exception('nologinas');
    }
