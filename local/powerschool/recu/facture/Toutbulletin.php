...
// vendor/autoload.php
require_once $CFG->dirroot."/local/powerschool/recu/facture/plusieures.php";
...
$idetudiants[]=(array)explode(',',$_GET["idetu"]);
$etu=new plusieurs();
// var_dump($idetudiants);die;
for($i=0;$i<=count($idetudiants);$i++)
// foreach($idetudiants as $key => $idetudiant)
{
    var_dump((int)$idetudiants[0][$i],$i);
        $sql1="SELECT * FROM {listenote} li,{affecterprof} af,{coursspecialite} co,{course} scou,{user} as u,{filiere} as fi,
                        ...
                        AND cse.idcoursspecialite=co.id AND li.idetudiant='".(int)$idetudiants[0][$i]."'";
        ...
        $sql="SELECT * FROM {listenote} li,{affecterprof} af,{coursspecialite} co,{course} scou,{courssemestre} cse WHERE af.id=li.idaffecterprof AND cse.id=af.idcourssemestre AND cse.idcoursspecialite=co.id
                        ...
                        AND li.idetudiant='".(int)$idetudiants[0][$i]."'";
        ...
        // var_dump($credit);die;
                ...
                // Générez le PDF
                $pdf_content=$html2pdf->output('output'.$idetudiants[$i].'.pdf',"S");
                ...
                $chemin_fichier = $chemin_dossier . 'output'.$idetudiants[$i].'.pdf';
                file_put_contents($chemin_fichier, $pdf_content);

        // // Le reste de votre code pour parcourir les tableaux et générer le PDF...
      
    //     $etu->generatePDF($idetudiant);
