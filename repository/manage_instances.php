...
$url = new powereduc_url('/repository/manage_instances.php');

$baseurl = new powereduc_url('/repository/manage_instances.php');
...
if ($context->contextlevel == CONTEXT_COURSE) {
    ...
    if ($USER->id != $context->instanceid){
        throw new \powereduc_exception('notyourinstances', 'repository');
    }
    $user = $USER;
} else {
...
if (isset($type)) {
    if (!$type->get_visible()) {
        throw new \powereduc_exception('typenotvisible', 'repository', $baseurl);
    }
    ...
    if (!$type->get_contextvisibility($context) || !$canviewrepository) {
        throw new \powereduc_exception('usercontextrepositorydisabled', 'repository', $baseurl);
    }
}
...
/// Process the form data if any, or display
    ...
    echo $OUTPUT->header();
    $formcontinue = new single_button(new powereduc_url($baseurl, array('delete' => $delete, 'sure' => 'yes')), get_string('yes'));
