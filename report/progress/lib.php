...
function report_progress_extend_navigation_course($navigation, $course, $context) {
    ...
    if($group===0 && $course->groupmode==SEPARATEGROUPS) {
        $showonnavigation = ($showonnavigation && has_capability('powereduc/site:accessallgroups', $context));
    }

    $completion = new completion_info($course);
    $showonnavigation = ($showonnavigation && $completion->is_enabled() && $completion->has_activities());
    if ($showonnavigation) {
        $url = new powereduc_url('/report/progress/index.php', array('course'=>$course->id));
